...
export class SpotifyMock {
  static withServerTime(serverTime = 1700000) {
    fetchMock.get('https://open.spotify.com/server-time', {
      serverTime: Math.floor(serverTime)
    });
  }
  static withAccessToken(accessToken = 'my-spotify-token') {
    SpotifyMock.withServerTime();
  ...
  static withSearchArtists(query: string, artists: Partial<SpotifyArtist>[], limit=10) {
    fetchMock.get(`https://api.spotify.com/v1/search?type=artist&q=${encodeURIComponent(query)}&limit=${limit}`, {
      artists: { items: artists } 
    });
  }
  
  static withGetTopArtist(query: string, artist: Partial<SpotifyArtist>) {
    fetchMock.get(`https://api.spotify.com/v1/search?type=artist&q=${encodeURIComponent(query)}&best_match=true&limit=1`, {
      best_match: { items: [artist] } 
    });
  }
  
  static withSearchArtistsFail(query: string) {
    fetchMock.get(`https://api.spotify.com/v1/search?type=artist&q=${encodeURIComponent(query)}&limit=1`, 500);
  }

  static withSearchReleases(query: string, releases: Partial<SpotifySimplifiedAlbum>[], limit=10) {
    fetchMock.get(`https://api.spotify.com/v1/search?type=album&q=${encodeURIComponent(query)}&limit=${limit}`, {
      albums: { items: releases } 
    });
  }
  
  static withSearchTracks(query: string, tracks: Partial<SpotifyTrack>[], limit=20) {
    fetchMock.get(`https://api.spotify.com/v1/search?type=track&q=${encodeURIComponent(query)}&limit=${limit}`, {
      tracks: { items: tracks } 
    });
  }
  ...
  static withSearchAll(query: string, artists: Partial<SpotifyArtist>[], releases: Partial<SpotifySimplifiedAlbum>[], tracks: Partial<SpotifyTrack>[]) {
    fetchMock.get(`https://api.spotify.com/v1/search?q=${encodeURIComponent(query)}&type=artist,album,track`, {
      artists: { items: artists },
      albums: { items: releases },
      tracks: { items: tracks }
    });
  ...
  static withGetTopAlbum(query: string, album: Partial<SpotifySimplifiedAlbum>) {
    fetchMock.get(`https://api.spotify.com/v1/search?type=album&q=${encodeURIComponent(query)}&best_match=true&limit=1`, {
